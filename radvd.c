...
main(int argc, char *argv[])
{
	...
	int opt_idx;
#endif
	syslog(LOG_ERR, "radvd: main starting...");
	...
	syslog(LOG_ERR, "radvd:start to get cmd conf_file(%s)",conf_file);
	syslog(LOG_ERR, "radvd:start to get cmd logfile(%s)",logfile);
	syslog(LOG_ERR, "radvd:start to get cmd pidfile(%s)",pidfile);
	/* parse args */
#define OPTIONS_STR "d:C:l:m:p:t:u:vhcsn"
...
#endif
	{
		syslog(LOG_ERR, "radvd: receive cmd:%c...", c);
		switch (c) {
		case 'C':
			conf_file = optarg;
			break;
		...
		case 'm':
			...
			else
			{
				...
				syslog(LOG_ERR, "%s: unknown log method: %s\n", pname, optarg);
				exit(1);
			}
			break;
		case 't':
		...
		case 's':
			...
			syslog(LOG_ERR, "privsep is not optional.  This options will be removed in a near future release.");
			break;
		case 'n':
			daemonize = 0;
			break;
		...
		case ':':
			...
			syslog(LOG_ERR, "%s: option %s: parameter expected\n", pname,
				prog_opt[opt_idx].name);
			exit(1);
#endif
		case '?':
	...
	syslog(LOG_ERR, "check chrootdir \n");
	if (chrootdir) {
		syslog(LOG_ERR, "check chrootdir(%s)\n", chrootdir);
		if (!username) {
			fprintf(stderr, "Chroot as root is not safe, exiting\n");
			syslog(LOG_ERR, "Chroot as root is not safe, exiting\n");
			free(chrootdir);
			exit(1);
		}

		syslog(LOG_ERR, "check username(%s)\n", username);
		if (chroot(chrootdir) == -1) {
			perror("chroot");
			syslog(LOG_ERR, "chroot");
			free(chrootdir);
			exit (1); 
		}

		syslog(LOG_ERR, "check username(%s) chdir\n", username);
		if (chdir("/") == -1) {
			perror("chdir");
		...
		syslog(LOG_ERR, "check username(%s) end\n", username);
		/* username will be switched later */
	}
	free(chrootdir);

	syslog(LOG_ERR, "configtest \n");
	if (configtest) {
		log_method = L_STDERR;
	}
	...
	if (sock < 0) {
		...
		syslog(LOG_ERR, "open_icmpv6_socket");
		exit(1);
	}

	syslog(LOG_ERR, "check_conffile_perm username(%s), conf_file(%s)\n", username, conf_file);
	...
	if (check_conffile_perm(username, conf_file) < 0) {
		if (get_debuglevel() == 0) {
			...
			syslog(LOG_ERR, "Exiting, permissions on conf_file invalid.\n");
			exit(1);
		}
		else
		{
			flog(LOG_WARNING, "Insecure file permissions, but continuing anyway");
			syslog(LOG_ERR, "Insecure file permissions, but continuing anyway");
		}
	}

	syslog(LOG_ERR, "check_ip6_forwarding\n");
	/* if we know how to do it, check whether forwarding is enabled */
	if (check_ip6_forwarding()) {
		flog(LOG_WARNING, "IPv6 forwarding seems to be disabled, but continuing anyway.");
	}

	syslog(LOG_ERR, "parse_config file(%s)\n", conf_file);
	/* parse config file */
	if (readin_config(conf_file) < 0) {
		flog(LOG_ERR, "Exiting, failed to read config file.\n");
		syslog(LOG_ERR, "Exiting, failed to read config file.\n");
		exit(1);
	}
	...
	syslog(LOG_ERR, "drop root privileges if request, username:%s \n", username);
	...
	syslog(LOG_ERR, "start to write pid file");
	write_pid_file(pidfile);
	syslog(LOG_ERR, "end to write pid file");
	/*
	...
	syslog(LOG_ERR, "sending stop adverts", pidfile);
	stop_adverts();
	flog(LOG_INFO, "removing %s", pidfile);
	syslog(LOG_ERR, "removing %s", pidfile);
	unlink(pidfile);
...
void write_pid_file(char const * pidfile)
{
	...
	syslog(LOG_ERR, "write_pid_file start");
	if ((fd = open(pidfile, O_RDONLY, 0)) >= 0)
	{
		...
		if (ret < 0)
		{
			...
			syslog(LOG_ERR, "cannot read radvd pid file, terminating: %s", strerror(errno));
			exit(1);
		}
		if (ret > 0) {
				pid_t pid;
				...
				if (pid > 0 && !kill(pid, 0)) {
					...
					syslog(LOG_ERR, "radvd already running, terminating.");
					exit(1);
				}
		}
		close(fd);
	...
	{
		syslog(LOG_ERR, "not atomic if pidfile is on an NFS mounted volume");
	...
	if (fd < 0)
	{
		...
		syslog(LOG_ERR, "cannot create radvd pid file, terminating: %s", strerror(errno));
		exit(1);
	}
	...
	if (ret != (int)strlen(pidstr))
	{
		...
		syslog(LOG_ERR, "cannot write radvd pid file, terminating: %s", strerror(errno));
		exit(1);
	}
...
void main_loop(void)
{
	...
	syslog(LOG_ERR, "main_loop start");
	...
	if (0 == sigsetjmp(jmpbuf, 1)) {
		canjump = 1;
		syslog(LOG_DEBUG, "mainloop, sigsetjmp OK");
	} else {
		syslog(LOG_DEBUG, "mainloop, siglongjmp triggerd");
		goto end;
	}
#endif
	...
	for (;;) {
		...
		syslog(LOG_ERR, "main_loop looping");
		if (IfaceList) {
			syslog(LOG_ERR, "main_loop IfaceList start");
			timeout = next_time_msec(IfaceList);
			next = IfaceList;
			for (iface = IfaceList; iface; iface = iface->next) {
				syslog(LOG_ERR, "main_loop loop iface(%s)", iface->Name);
				int t;
		...
		syslog(LOG_ERR, "polling for %g seconds.", timeout/1000.0);

		rc = poll(fds, sizeof(fds)/sizeof(fds[0]), timeout);

		if (rc > 0) {
			dlog(LOG_DEBUG, 5, "main_loop, rc > 0");
			syslog(LOG_ERR, "main_loop, rc > 0");
			if (fds[0].revents & (POLLERR | POLLHUP | POLLNVAL)) {
				flog(LOG_WARNING, "socket error on fds[0].fd");
				syslog(LOG_ERR, "socket error on fds[0].fd");
			}
				...
				syslog(LOG_ERR, "main_loop recv rs_ra len =%d", len);
				if (len > 0) {
			...
			if (fds[1].revents & (POLLERR | POLLHUP | POLLNVAL)) {
				...
				syslog(LOG_ERR, "socket error on fds[1].fd");
			}
			...
			syslog(LOG_ERR, "poll returned early: %s", strerror(errno));
		}

		if (sigterm_received || sigint_received) {
			flog(LOG_WARNING, "Exiting, sigterm or sigint received.\n");
			syslog(LOG_ERR, "Exiting, sigterm or sigint received.\n");
			break;
		}
		...
		if (sighup_received)
		{
			...
			syslog(LOG_ERR, "sig hup received.\n");
			reload_config();
			sighup_received = 0;
		}
		...
		if (sigusr1_received)
		{
			...
			syslog(LOG_ERR, "sig usr1 received.\n");
			reset_prefix_lifetimes();
			sigusr1_received = 0;
		}
		syslog(LOG_ERR, "main_loop looping end");
	}
end:
	syslog(LOG_ERR, "main_loop end");
}
...
timer_handler(void *data)
{
	...
	syslog(LOG_DEBUG, "timer_handler called for %s", iface->Name);
...
void reload_config(void)
{
	...
	syslog(LOG_ERR, "attempting to reread config file");
	...
	while(iface)
	{
		...
		syslog(LOG_ERR, "freeing interface %s", iface->Name);
	...
	if (readin_config(conf_file) < 0) {
		...
		syslog(LOG_ERR, "readin_config(%s) failed.", conf_file);
		exit(1);
	}
...
#if USE_JUMP
	if (0 == canjump) {
		syslog(LOG_ERR, "sigterm_handler, canjump = 0, do nothing");
	} else {
		canjump = 0;
		syslog(LOG_DEBUG, "sigterm_handler: to siglongjmp ...");
		siglongjmp(jmpbuf, 1);
	}
#endif
	...
	for (iface = IfaceList; iface; iface = iface->next) 
	{
							...
							prefix = prefix->next) 
		{
			if (prefix->DecrementLifetimesFlag)
			{
				...
				syslog(LOG_DEBUG, "%s/%u%%%s plft reset from %u to %u secs", pfx_str, prefix->PrefixLen, iface->Name, prefix->curr_preferredlft, prefix->AdvPreferredLifetime);
				syslog(LOG_DEBUG, "%s/%u%%%s vlft reset from %u to %u secs", pfx_str, prefix->PrefixLen, iface->Name, prefix->curr_validlft, prefix->AdvValidLifetime);
...
check_conffile_perm(const char *username, const char *conf_file)
{
	...
	syslog(LOG_ERR, "readin_config(%s)", conf_file);
	if (fp == NULL) {
		flog(LOG_ERR, "can't open %s: %s", conf_file, strerror(errno));
		syslog(LOG_ERR, "can't open %s: %s", conf_file, strerror(errno));
		return (-1);
	}
	fclose(fp);
	...
	{
		syslog(LOG_ERR, "stat conf_file failed");
		return (-1);
	}

	if (stbuf.st_mode & S_IWOTH) {
                flog(LOG_ERR, "Insecure file permissions (writable by others): %s", conf_file);
                syslog(LOG_ERR, "Insecure file permissions (writable by others): %s", conf_file);
		return (-1);
        }
	...
	if (strncmp(username, "root", 5) != 0 &&
	    ((stbuf.st_mode & S_IWGRP && pw->pw_gid == stbuf.st_gid) ||
	     (stbuf.st_mode & S_IWUSR && pw->pw_uid == stbuf.st_uid))) {
                ...
                syslog(LOG_ERR, "Insecure file permissions (writable by self/group): %s", conf_file);
		return (-1);
        }
...
readin_config(char *fname)
{
	...
	if (yyparse() != 0)
	{
		...
		syslog(LOG_ERR, "error parsing or activating the config file: %s", fname);
		return (-1);
	}
